...
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine new_fog(nv,ifunit,ipunit,jpdtn,jf,im,jm,dx,dy,dt,
     +     iens,Lm,Lp,Lt,derv_mn,derv_sp,derv_pr,wgt)
...
c          jpdtnp=8    !Temperature variation in prcip file is in Template 4.8
          ...
          jpdtnp=0    !Jun Du: added a new code and use 0 for rrfs and gefs Template 4.0 (Sec 4)
...
c         call readGB2(ifunit(irun),jpdtn,3,5,2,0,jp27,gfld,eps,ie) !CLDBS
          call readGB2(ifunit(irun),jpdtn,3,5,215,0,jp27,gfld,eps,ie) !Ceiling
...
c         call readGB2(ifunit(irun),jpdtn,3,5,3,0,jp27,gfld,eps,ie) !CLDT
          call readGB2(ifunit(irun),jpdtn,3,5,215,0,jp27,gfld,eps,ie) !Ceiling
          if(ie.eq.0) then
           CLDTP(:,irun)=gfld%fld
          else
...
c Temperature variation (pre-calculated by J. Du) in prcip file is in Template 8
...
c Read in past T
        write(*,*) 'ifunit,ipunit,lvl,jpdtn,jpdtnp,jp27 ',
     +              ifunit,ipunit,lvl,jpdtn,jpdtnp,jp27
        jp27=-9999
