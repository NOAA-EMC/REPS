...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     +             loutput
       ...
       IENS=0
       do irun=1,50
c       if(ifhr.lt.100) then
c        read(1,301,END=302) weight(irun),files(irun),mbrname(irun)
c       else
c        read(1,303,END=302) weight(irun),files(irun),mbrname(irun)
c       end if
...
c        if( files(irun)(1:4).ne.'gefs' ) then       
          mbrname(irun)=files(irun)
         end if
c        write(*,301) weight(irun), files(irun),mbrname(irun)
         write(*,304) weight(irun), files(irun),mbrname(irun)
         IENS=IENS+1
         prcps(irun)='prcip'//files(irun)(5:18)              !get prcip file names
...
c        if(itime.lt.100) then
c         write(cfhr,'(i2.2)') itime
c        else
...
CCCC Binbin Zhou Note:
       ...
       write(*,*) 'ID,im,jm,jf,dx,dy =', gribid,im,jm,jf,dx,dy
...
c      if(trim(eps).eq.'sref'.or.trim(eps).eq.'gefs') then
c       if (.not.allocated(bmap_f)) then
         allocate(bmap_f(jf))
c       end if
c      end if
...
!  end of inserted ptype stuff
         ...
         write(0,*) 'get APCP or SNOW GRIB2 data for member ', irun

             if(mbrname(irun)(1:4).eq.'sref'.or.
     +          mbrname(irun)(1:4).eq.'gefs') then

! In GEFS, precip and WEASD used different jpdtn numbers  (J. Du)
              if (vname(nv).eq.'AP1h'.or.vname(nv).eq.'AP3h'.or. 
     &            vname(nv).eq.'AP6h'.or.vname(nv).eq.'AP12h'.or.
     &            vname(nv).eq.'AP24h'.or.
     &            vname(nv).eq.'A1RI'.or.vname(nv).eq.'A3RI'.or. 
     &            vname(nv).eq.'A6RI'.or.vname(nv).eq.'A12R'.or.
     &            vname(nv).eq.'A24R'.or.
     &            vname(nv).eq.'FFG1'.or.vname(nv).eq.'FFG3'.or. 
     &            vname(nv).eq.'FFG6'.or.vname(nv).eq.'FF12'.or.
     &            vname(nv).eq.'FF24') then
               jpdtn=11
              endif
              if (vname(nv).eq.'SN1h'.or.vname(nv).eq.'SN3h'.or. 
     &            vname(nv).eq.'SN6h'.or.vname(nv).eq.'SN12h'.or.
     &            vname(nv).eq.'SN24h') then
               jpdtn=1
              endif
              if (vname(nv).eq.'WEAS'.or.vname(nv).eq.'WEASD') then 
               jpdtn=1
              endif
	...
	write(0,*) 'jpd1 defined(b): ', jpd1
     ...
     +         (k4(nv).eq.2.and.k5(nv).eq.221).or.
     +         (k4(nv).eq.6.and.k5(nv).eq.1) ) then  !added cloud cover - J. Du
                if(mbrname(irun)(1:4).eq.'sref'.or.
     +             mbrname(irun)(1:4).eq.'gefs') then
                 jpdtn=11
                else 
...
c   Loop 2-0: allocation
	...
	write(0,*) 'jpd1 defined(c): ', jpd1
         ...
         if(trim(eps).eq.'sref'.or.trim(eps).eq.'gefs') then
           if (jpd1.eq.1.and.
...
c ptype's jpdtn=11 for gefs (J. Du)
              if(trim(eps).eq.'gefs') then 
               jpdtn=11
              endif
     ...
     +         ptype_mn,ptype_pr,ptype_pr2)
              ...
              do jp=1,4
               ...
               jpd12=0
c              jpd27=-9999
               jpd27=6    !GEFS.v12 is in 6 hour interval (J. Du)
...
c           if(dk4(nv).eq.1.and.dk5(nv).eq.13.and.
c    +       trim(eps).eq.'rrfs') then
            if(dk4(nv).eq.1.and.dk5(nv).eq.15.and.
     +       trim(eps).eq.'gefs') then
...
! Reset gfld%bmap with the combined version bmap_f
     ...
     +          associated(gfld%bmap)) gfld%bmap=bmap_f
          ...
          write(*,*) 'before packGB2_mean_derv'
          if (dvname(nv).eq.'SN1h') then
             jpd27=1
             write(*,*) 'before packGB2_mean_derv for SN1h'
           else if (dvname(nv).eq.'SN3h') then
             jpd27=3
             write(*,*) 'before packGB2_mean_derv for SN3h'
           else if (dvname(nv).eq.'SN6h') then
             jpd27=6
             write(*,*) 'before packGB2_mean_derv for SN6h'
           else if (dvname(nv).eq.'SN12') then
             jpd27=12
             write(*,*) 'before packGB2_mean_derv for SN12'
           else if (dvname(nv).eq.'SN24') then
             jpd27=24
             write(*,*) 'before packGB2_mean_derv for SN24'
          end if
	...
	write(0,*) 'calling packGB2_prob_derv here'
          ...
          write(*,*) 'before packGB2_prob_derv'
               call packGB2_prob_derv(iprob,derv_pr,
     +              nv,jpd1,jpd2,jpd10,jpd27,jf,Lp,Lth,
     +              iens,iyr,imon,idy,ihr,ifhr,gribid,bmap_f,gfld)  !borrow gfld from what was got from previous direct variables
...
c 3-4: deallocate
     ...
     +      (jpd2.eq.8.or.jpd2.eq.11.or.jpd2.eq.15)) then
              jpdtn=11
           elseif (jpd1.eq.6.and.jpd2.eq.1) then
              jpdtn=11
           else
              jpdtn=1
           end if
...
c J. Du - Added this section to actually control jpdtn for special variables
          if(trim(eps).eq.'sref'.or.trim(eps).eq.'gefs') then
           if (qk4(nv).eq.1.and.
     +      (qk5(nv).eq.8.or.qk5(nv).eq.11.or.qk5(nv).eq.15)) then
              jpdtn=11
           elseif (qk4(nv).eq.6.and.qk5(nv).eq.1) then
              jpdtn=11
           else
              jpdtn=1
           end if
     ...
     +      (qk5(nv).eq.8.or.qk5(nv).eq.11.or.qk5(nv).eq.15)) then
         ...
         write(*,*) 'before call get_mxp for ', nv
         call get_mxp(nv,ifunit,jpdtn,jf,iens,Lq,mxp8,weight) 
c        call get_mxp(nv,ifunit,jpdtn,jf,iens,Lq,mxp8,weight,eps)
